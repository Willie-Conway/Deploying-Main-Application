...
# URL configurations for the Songs and Photos microservices
SONGS_URL = "http://songs-sn-labs-hirewillieco.labs-prod-openshift-san-a45631dc5778dc6371c67d206ba9ae5c-0000.us-east.containers.appdomain.cloud"  # Songs microservice URL
PHOTO_URL = "https://pictures.1t07bpn0l6uk.us-south.codeengine.appdomain.cloud"  # Photos microservice URL
...
def songs(request):
    # Use the SONGS_URL to fetch songs from the Songs microservice
    songs = requests.get(f"{SONGS_URL}/song").json()
    return render(request, "songs.html", {"songs": songs["songs"]})

def photos(request):
    # Use the PHOTO_URL to fetch photos from the Photos microservice
    photos = requests.get(f"{PHOTO_URL}/picture").json()
